...
#| tbl-cap: "Summary of Model Results (N = 185,504)"
#| echo: false
#| warning: false
library(kableExtra)
library(janitor)
...
model_summary_table |>
    ...
    mutate(
        "Coefficient (95% CI)" = paste0(
            coef_w_stars,
            " (",
            round(x95_percent_ci_lower, 3),
            ", ",
            round(x95_percent_ci_upper, 3),
            ")"
        )
    ) |> 
    rename(
...
model_1_p = model_summary_table |>
    filter(Model == "Model 1: ICE Presence") |>
    pull("P-value")
```

@tbl-model-summary provides a summary of the results from our three regression models. The first model, which estimates the effect of the presence of an ICE detention facility in a ZCTA on doctor visits, shows a negative, but insignificant impact (p = `r model_1_p`). The second model, which estimates the effect of...

# Discussion

This study has several limitations. First, we did not account for access to health care facilities in our models (e.g., distance to the nearest hospital or clinic), which may confound the relationship between enforcement and health care utilization. Second, our outcome data contain significant geographic missingness, particularly in Kentucky, Pennsylvania, and much of the western United States, which may introduce bias. Third, very few ZCTAs have a detention facility present, so the data are highly unbalanced and most observations have no ICE detention facility nearby. This limited exposure variation makes it harder to identify stable effects and may reduce statistical power. Finally, our main independent variable—the presence of an ICE detention facility—is a relatively blunt measure of enforcement intensity and may not capture the full range of enforcement activities affecting health care utilization, such as local raids or workplace enforcement operations. Future research should incorporate more granular measures of enforcement activity, including local ICE raids and workplace enforcement actions, along with additional controls for health care access such as hospital proximity and provider availability. Improving data completeness and geographic coverage would also strengthen the robustness of future analyses.
